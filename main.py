...
async def start_command(client: Client, message: Message):
    ...
    if await is_admin(user.id):
        await message.reply_text(
            ...
            "• <code>/delete FILE_ID</code> — Delete a file\n"
...
async def file_upload(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        f"🗑️ Use <code>/delete {file_id}</code> to delete this file.",
        parse_mode=enums.ParseMode.HTML,
        disable_web_page_preview=True,
...
async def files_command(client: Client, message: Message):
    ...
    for i, f in enumerate(files[:20], 1):
        name = (f.get("file_name") or "Unnamed")[:40]
        ftype = (
        ...
        text += (
            ...
            f"   <code>/views {f['file_id']}</code>\n"
            f"   <code>/delete {f['file_id']}</code>\n\n"
        )
...
async def delete_file_command(client: Client, message: Message):
    """Handle /delete FILE_ID — delete a file and all its records (admin only)."""
    if not await is_admin(message.from_user.id):
        await message.reply_text(
            "❌ You are not authorized to use this command."
        )
    ...
    if len(parts) < 2:
        await message.reply_text(
            ...
            "Get the FILE_ID from the upload confirmation or <code>/files</code> command.",
            parse_mode=enums.ParseMode.HTML,
        )
        return
    ...
    file_name = file_data.get("file_name") or "Unnamed"
    unique_id = file_data.get("unique_id", "N/A")

    success = delete_file(file_id)

    if success:
        await message.reply_text(
            f"✅ <b>File Deleted Successfully</b>\n\n"
            f"📄 <b>Name:</b> <code>{file_name}</code>\n"
            f"🔗 <b>Unique ID:</b> <code>{unique_id}</code>\n"
            f"🗑️ All views and download records have also been removed.",
            parse_mode=enums.ParseMode.HTML,
        )
        logger.info(f"Admin deleted file: {file_name} ({file_id[:16]}...)")
    else:
        await message.reply_text(
...
def register_handlers(client: Client):
    ...
    client.add_handler(
        MessageHandler(
            delete_file_command, filters.command("delete")
        ),
    )
...
async def health_check():
    """Health-check endpoint — supports GET, HEAD, OPTIONS, POST for UptimeRobot."""
    return {
        "status": "running",
